...
class HomeController extends Controller
{
    ...
    public function knowledge($id)
    {
        ...
        if ($articleslug != null) {
            ...
            $like = article_likes::where('rating', '1')
                ->where(['article_id' => $like_post->id])
                ->paginate();
            $data['like'] = $like;

            $dislike = article_likes::where('rating', '-1')
                ->where(['article_id' => $like_post->id])
                ->paginate();
            $data['dislike'] = $dislike;

            if (Auth::guard('customer')->check()) {
                $viewrating = article_likes::where('user_id', Auth::guard('customer')->user()->id)
                    ->where(['article_id' => $like_post->id])
                    ->first();
        ...
        $populararticle = Article::orderBy('views', 'desc')
            ->latest()
            ->paginate(5);
                    ...
                    if ($row->status == 'Published') {
                        if ($row->articleslug != null) {
                            $output .=
                                ...
                                '<span class="badge bg-success-transparent text-success float-end fs-11">' .
                                Str::limit($row->category->name, '20') .
                                '</span></a>
                        </li>';
                        } else {
                            $output .=
    ...
    public function setLanguage($locale)
    {
        ...
        return redirect()
            ->back()
            ->with('success', trans('langconvert.functions.languageupdate'));
    }
        ...
        if ($request->get('data')) {
            ...
            $output = '<div class="card sprukocard wow shake" data-wow-duration="1500ms">
                    ...
                    <div class= "card-body">
                        ...
                        <ul class="item-list item-list-scroll custom-ul mb-0">
                ';
            if ($data->isNotEmpty()) {
                foreach ($data as $row) {
                    $output .= '<li class="item position-relative mb-4">';
