    ...
    <div class="cover-image sptb">
        <div class="container">
            <div class="row">
                            ...
                            <div class="card-body item-user text-center">
                                <div class="profile-pic">
                                    <div class="profile-pic-img mb-2">
                                        ...
                                        @else
                                        <img class="brround avatar-xxl" alt="{{ Auth::user()->image }}" src="{{ asset('uploads/profile/' . Auth::user()->image) }}">
                                        @endif

                                    </div>
                                    <a href="#" class="text-dark">
                                        <h5 class="font-weight-semibold2 mb-1">{{ Auth::user()->username }}</h5>
                                    </a>
                        ...
                        <div class="purchasecodes alert alert-light-warning br-13 mb-5">
                            <div class="ps-0 pe-0 pb-0">
                                <div class="">
                                    ...
                                    <p class="mb-0 mt-3">
                                        <small>Your support for this item has expired. You may still leave a
                                            comment but please renew support if you are asking the author for
                                            help. View the item support policy</small>
                                    </p>
                                    @else
                                    @endif
                        ...
                        <div class="card">
                            <div class="card-header border-0">
                                <div class="card-title">
                                    {{ trans('langconvert.admindashboard.ticketinformation') }}
                                </div>
                                <input type="hidden" name="" data-id="{{ $ticket->id }}" id="ticket">
                                <div class="float-end ms-auto"><a href="{{ route('client.ticket') }}" class="btn btn-white btn-sm ms-auto"><i class="fa fa-paper-plane-o me-2 fs-14"></i>{{ trans('langconvert.adminmenu.createticket') }}</a>
                                </div>
                            </div>
                            <div class="card-body px-0 pt-2 pb-0">
                                <div class="table-responsive tr-lastchild">
                                    <table class="table-information mb-0 table">
                                        <tbody>
                                            <tr>
                                                <td>
                ...
                <div class="col-xl-8">
                    <div class="card">
                        <div class="card-header d-block mb-1 border-0">
                            <div class="d-sm-flex d-block">
                                <div>
                                    <h4 class="card-title fs-22 mb-1">{{ $ticket->subject }} </h4>
                                </div>
                            ...
                            <small class="fs-13"><i class="feather feather-clock text-muted me-1"></i>{{ trans('langconvert.admindashboard.lastupdatedon') }}
                                <span class="text-muted">{{ $ticket->updated_at->diffForHumans() }}</span></small>
                        </div>
                        <div class="card-body readmores mx-1 px-6 pt-2">
                            <div>
                                ...
                                <div class="row galleryopen">
                                    ...
                                    <div class="file-image-1 removespruko{{ $ticketss->id }}" id="imageremove{{ $ticketss->id }}">
                                        <div class="product-image">
                                            <a href="{{ $ticketss->getFullUrl() }}" class="imageopen" target="_blank">
                        ...
                        <form method="POST" action="{{ url('customer/closed/' . $ticket->ticket_id) }}">
                            ...
                            <div class="card-body">
                                <p>{{ trans('langconvert.userdashboard.ticketclosedreopen') }}
                                    <input type="submit" class="btn btn-secondary" value="{{ trans('langconvert.admindashboard.reopen') }}" onclick="this.disabled=true;this.form.submit();">
                                </p>
                            </div>
                        </form>
                    </div>
                    ...
                    @if (now()->format('Y-m-d') <= $ticket->closing_ticket->adddays(setting('USER_REOPEN_TIME'))->format('Y-m-d'))
                        ...
                        <div class="alert alert-light-warning note" role="alert">
                            <p class="m-0"><b>{{ trans('langconvert.admindashboard.note') }}:-</b>
                                {{ $ticket->note }}
                            </p>
                        </div>
                        @else
                        <div class="card">
                            ...
                            <form method="POST" action="{{ route('client.comment', $ticket->ticket_id) }}" enctype="multipart/form-data">
                                @csrf
                                @honeypot
                                ...
                                <div class="card-body">
                                    ...
                                    <div class="custom-controls-stacked d-md-flex mt-3">
                                        <label class="form-label me-5 mt-1">{{ trans('langconvert.admindashboard.status') }}</label>
                                        <label class="custom-control form-radio success me-4">
                                            @if ($ticket->status == 'Re-Open')
                                            <input type="radio" class="custom-control-input" name="status" value="Inprogress" {{ $ticket->status == 'Re-Open' ? 'checked' : '' }}>
                                            <span class="custom-control-label">{{ trans('langconvert.newwordslang.inprogress') }}</span>
                                            @elseif($ticket->status == 'Inprogress')
                                            <input type="radio" class="custom-control-input" name="status" value="{{ $ticket->status }}" {{ $ticket->status == 'Inprogress' ? 'checked' : '' }}>
                                            <span class="custom-control-label">{{ trans('langconvert.newwordslang.leaveascurrent') }}</span>
                                            @else
                                            <input type="radio" class="custom-control-input" name="status" value="{{ $ticket->status }}" {{ $ticket->status == 'New' ? 'checked' : '' }}>
...
<script src="{{ asset('assets/plugins/jquerysticky/jquery-sticky/jquery-sticky.js') }}?v=<?php echo time(); ?>">
</script>
