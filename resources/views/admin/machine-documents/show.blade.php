...
@section('content')
    ...
    <!-- Machine Document Edit -->
    <div class="col-xl-12 col-lg-12 col-md-12">
        <div class="card">
            <div class="card-header border-0">
                <h4 class="card-title">{{ trans('langconvert.machines.edit_machine_document') }}</h4>
            </div>
            <form method="POST" action="{{ url('/admin/machine-documents/' . $document->id) }}"
                    ...
                    <div class="row">
                        ...
                        <label class="form-label">{{ trans('langconvert.machines.document_type') }} <span class="text-red">*</span></label>
                            <select class="form-control select2-show-search select2 @error('type') is-invalid @enderror"
                                ...
                                <option value='document' @if ($document->type == 'document') selected @endif>{{ trans('langconvert.machines.pdf_word_document') }}</option>
                        ...
                        <div class="form-group">
                            ...
                            <select
                                ...
                                @foreach ($machines as $machine)
                                    <option value="{{ $machine->id }}" @if ($document->machine_id == $machine->id) selected @endif>
                                        {{ getName($machine) }} (
